"""
SGS .ply → RTR-GS .ply 转换脚本

将 SGS 训练输出的 .ply 文件转换为 RTR-GS 格式，
将 RTR-GS 扩展属性初始化为默认值。

用法:
    python script/sgs2rtrgs.py <sgs_ply_path> <output_path>
    python script/sgs2rtrgs.py <sgs_ply_path> <output_path> --sh_degree 3

也可以直接传入模型路径和迭代次数:
    python script/sgs2rtrgs.py -m <sgs_model_path> -i 30000 -o <output_path>
"""
import argparse
import os
import sys
...
def load_sgs_ply(path, sh_degree=3):
    """加载 SGS 格式的 .ply 文件"""
...
def convert_sgs_to_rtrgs(xyz, features_dc, features_extra, opacities, scales, rots, sh_degree=3):
    """将 SGS 属性转换为 RTR-GS 格式，添加默认的扩展属性"""
    N = xyz.shape[0]
...
def main():
    parser = argparse.ArgumentParser(description="Convert SGS .ply to RTR-GS .ply")
    parser.add_argument("input_ply", nargs="?", type=str, default=None,
                        help="Path to SGS .ply file")
    parser.add_argument("output_ply", nargs="?", type=str, default=None,
                        help="Path to output RTR-GS .ply file")
    parser.add_argument("--sh_degree", type=int, default=3,
                        help="SH degree used in SGS training (default: 3)")
    parser.add_argument("-m", "--model_path", type=str, default=None,
                        help="SGS model path (alternative to input_ply)")
    ...
    print(f"Loading SGS PLY from: {input_ply}")
    xyz, f_dc, f_rest, opacities, scales, rots = load_sgs_ply(input_ply, args.sh_degree)
    print(f"Loaded {xyz.shape[0]} points")

    print("Converting to RTR-GS format...")
    attributes = convert_sgs_to_rtrgs(xyz, f_dc, f_rest, opacities, scales, rots, args.sh_degree)
