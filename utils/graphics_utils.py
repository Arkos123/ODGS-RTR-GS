...
def fibonacci_sphere_sampling(normals, sample_num, random_rotate=True):
    ...
    rotation_matrix = rotation_between_z(normals).reshape(*pre_shape, 3, 3)
...
def latlong_to_cubemap(latlong_map: torch.Tensor, res: List[int]) -> torch.Tensor:
    ...
    for s in range(6):
        ...
        phi = torch.arccos(v[..., 2:3])
        theta = torch.atan2(v[..., 1:2], v[..., 0:1])
        tv = (phi / np.pi) 
        tu = -theta / (2 * np.pi) + 0.5
...
def latlong_to_cubemap_orb(latlong_map, res):
    cubemap = torch.zeros(6, res[0], res[1], latlong_map.shape[-1], dtype=torch.float32, device='cuda')
    for s in range(6):
        gy, gx = torch.meshgrid(torch.linspace(-1.0 + 1.0 / res[0], 1.0 - 1.0 / res[0], res[0], device='cuda'), 
                                torch.linspace(-1.0 + 1.0 / res[1], 1.0 - 1.0 / res[1], res[1], device='cuda'),
                                indexing='ij')
        v = F.normalize(cube_to_dir_orb(s, gx, gy), p=2, dim=-1)
...
def latlong_to_cubemap_nvdiffrec(latlong_map, res):
    cubemap = torch.zeros(6, res[0], res[1], latlong_map.shape[-1], dtype=torch.float32, device='cuda')
    for s in range(6):
        gy, gx = torch.meshgrid(torch.linspace(-1.0 + 1.0 / res[0], 1.0 - 1.0 / res[0], res[0], device='cuda'), 
                                torch.linspace(-1.0 + 1.0 / res[1], 1.0 - 1.0 / res[1], res[1], device='cuda'),
